import React from "react";

function About() {
  return (
    <section className='container max-w-[784px] mt-24' id='about-faraday'>
      <h2 className='text-center text-xl md:text-2xl font-bold'>About</h2>

      <p className='mt-4 text-center max-w-[40rem] mx-auto'>
        Faraday is a community driven ed-tech solution targeted towards tertiary
        ínstitution students to help improve the learning experience. We're
        working tirelessly to build a community where scholars can share what
        they know with others who need help.
      </p>
    </section>
  );
}

export default About;
